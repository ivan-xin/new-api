...
export default function Web3Payment({ amount, visible, onCancel, onSuccess }) {
  ...
  const fetchPayInfo = async () => {
    try {
      ...
      if (res.data.message === 'success') {
        ...
        Toast.error(res.data.data || '获取支付信息失败');
      }
    } catch (error) {
  ...
  const handlePay = async () => {
    ...
    if (!USDC_CONTRACTS[chainId]) {
      Toast.error('当前网络不支持 USDC 支付，请切换网络');
      return;
    }
    ...
    try {
      ...
      if (userBalance < amountToSend) {
        throw new Error(`USDC 余额不足，需要 ${orderData.amount_usdc} USDC`);
      }
      ...
      Toast.info(`交易已提交！哈希: ${tx.hash.substring(0, 10)}...`);
      Toast.info('等待区块确认中，请稍候...');
      ...
      if (verifyRes.data.message === 'success') {
        ...
        Toast.info('交易正在确认中，请稍后刷新页面查看');
        if (onSuccess) {
          onSuccess();
        }
      } else {
        throw new Error(verifyRes.data.data || '验证失败');
      }
    } catch (error) {
    ...
    if (!wallet) {
      return (
        <div style={{ textAlign: 'center', padding: '40px 20px' }}>
          ...
          <Title heading={4}>选择钱包连接</Title>
          <Text type="tertiary">支持 MetaMask、WalletConnect、Coinbase Wallet 等</Text>
          <Button
            theme="solid"
            type="primary"
            size="large"
    ...
    return (
      <Space vertical spacing="loose" style={{ width: '100%' }}>
        ...
        {!USDC_CONTRACTS[chainId] && (
          ...
          >
            <Text type="warning">⚠️ 当前网络不支持 USDC 支付</Text>
            <br />
            <Text type="tertiary" size="small">
              请切换到 Base、BSC、Arbitrum 或 Ethereum 网络
            </Text>
            <br />
        ...
        {payInfo && parseFloat(balance) < parseFloat(payInfo.amount_usdc) && (
          ...
          >
            <Text type="danger">⚠️ USDC 余额不足</Text>
            <br />
            <Text type="tertiary" size="small">
              需要: {payInfo.amount_usdc} USDC
              <br />
              当前: {parseFloat(balance).toFixed(2)} USDC
            </Text>
          </Card>
        )}
